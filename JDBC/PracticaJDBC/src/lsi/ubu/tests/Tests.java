...
public class Tests {
	...
	public void ejecutarTestsAnularBilletes() {
		...
		try {
	        ...
	        // Parámetros para la anulación del billete
	        ...
	        int plazasLibresAntes = 0;

	        int plazasLibresDespues = 0;

	        int vIdViaje = 0;



	        // Obtener el número de plazas libres antes de la anulación

	        st = con.prepareStatement("SELECT nPlazasLibres, idViaje FROM viajes JOIN tickets USING (idViaje) WHERE idTicket = ?");
	        ...
	        if (rs.next()) {
	            ...
	            plazasLibresAntes = rs.getInt("nPlazasLibres");

	            vIdViaje = rs.getInt("idViaje");
	        ...
	        // Obtener el número de plazas después antes de la anulación

	        st = con.prepareStatement("SELECT nPlazasLibres FROM viajes WHERE idViaje = ?");

	        st.setInt(1, vIdViaje);
	        ...
	        if (rs.next()) {
	            ...
	            plazasLibresDespues = rs.getInt("nPlazasLibres");
	        ...
	        if (count > 0) {
	        	...
	        	if (plazasLibresDespues == plazasLibresAntes + nroPlazas ) {

	        		LOGGER.info("Anular billete BIEN");

	        	}else {

	        		LOGGER.info("No se han actualizados las plazas libres después de la reserva MAL");
